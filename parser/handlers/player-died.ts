...
const handler:EventHandler = {
  regex: /^\[([0-9.:-]+)]\[([ 0-9]*)]LogSquadTrace: \[DedicatedServer](?:ASQSoldier::)?Die\(\): Player:(.+) KillingDamage=-*([0-9.]+) from ([A-z_0-9]+) \(Online IDs: EOS: (\w{32}) steam: (\d{17}) \| Contoller ID: (\w+)\) caused by ([A-z_0-9-]+)_C/,
